...
# 🚀 TURBO MODE: Povećan broj radnika jer je assortment endpoint brz i stabilan
FETCH_WORKERS = 15
...
st.set_page_config(page_title="Promo Monitor – Turbo", page_icon="🏷️", layout="wide")
...
def is_ignored_promo(text: str) -> bool:
    ...
    t = t.replace("[wolt+]", "").replace("[sniženje]", "").strip()
...
def load_amm() -> pd.DataFrame: return load_amm_github()
def save_amm(df: pd.DataFrame): save_amm_github(df)
def load_alert_log() -> pd.DataFrame: return load_alert_log_github()
def save_scan(df: pd.DataFrame): save_scan_github(df)
def load_scan() -> pd.DataFrame: return load_scan_github()
def scan_meta() -> str | None: return scan_meta_github()
...
def _keepalive_loop():
    time.sleep(30)
    while True:
        try: requests.get("http://localhost:8501/_stcore/health", timeout=10)
...
def make_thread_session() -> requests.Session:
    ...
    for k, v in session.headers.items():
        s.headers[k] = v
    try: cookie_val = Path("_scan_cookie.txt").read_text().strip()
    except Exception: cookie_val = ""
    if not cookie_val: cookie_val = WOLT_COOKIE or ""
    if cookie_val: s.headers["Cookie"] = cookie_val
    return s
...
def _wait_throttle():
    now = time.time()
    with _throttle_lock: wait = _throttle_until - now
...
def _fetch_url(ts, url: str, label: str, stop_event) -> tuple:
    for attempt in range(4):
        ...
        _wait_throttle()
        try:
            # Smanjen jitter pošto koristimo brži endpoint
...
# 🚀 TURBO LOGIKA: Umesto dynamic-a, gadjamo ASSORTMENT!
def _fetch_one(slug: str, lat: float, lon: float, feed_akcije: list, stop_event: threading.Event) -> tuple[str, str]:
    ...
    akcije_str = "-"
    data, _ = _fetch_url(ts, url, f"ASSORT {slug}", stop_event)
    ...
    if data:
        try:
            for item in data.get("items", []):
                ...
                # Provera da li je item snižen
                if stara_cena and nova_cena and stara_cena > nova_cena:
                    sc_rsd = stara_cena // 100
                    nc_rsd = nova_cena // 100
                    ime = item.get("name", "Artikal")
                    item_promos.append(f"• [Sniženje] {ime} ({sc_rsd} -> {nc_rsd} RSD)")
        except Exception as e:
            _log_fetch(f"ASSORT {slug} → parse EXC {e}")
            
    # Kombinujemo opšte akcije (dobijene iz feed-a) i konkretna jela na sniženju
    combined = list(dict.fromkeys(feed_akcije + item_promos))
...
def _update_city_progress(city_display: str, found: int = None, total: int = None, status: str = None):
    with _city_progress_lock:
        ...
        if found is not None: _city_progress[city_display]["found"] = found
        if total is not None: _city_progress[city_display]["total"] = total
        if status is not None: _city_progress[city_display]["status"] = status
...
def fetch_city(city_display: str, status_placeholder, stop_event: threading.Event) -> list[dict]:
    ...
    # ── Faza 1: Skupljamo sve restorane i njihove feed promocije ────────────────
    for loc_idx, (lat, lon) in enumerate(multi_coords):
        ...
        for page_num in range(50):
            ...
            if data:
                for section in data.get("sections", []):
                    for item in section.get("items", []):
                        ...
                        # 🚀 TURBO: Čitamo opšte promocije direktno iz glavnog feed-a!
                        for promo in venue.get("promotions", []):
                            txt = promo.get("text") or promo.get("title", "")
                            if txt: feed_akcije.append(f"• {txt}")

                        # Čitamo bedževe (za Novo i ostale opcije)
                        for badge in venue.get("badges", []):
                            txt = badge.get("text", "")
                            if txt:
                                if txt.lower() in ["novo", "new"]: novo_status = "Da"
                                else: feed_akcije.append(f"• {txt}")
                                
                        label = venue.get("label", "")
                        if label:
                            if label.lower() in ["novo", "new"]: novo_status = "Da"
            ...
            if items_in_response == 0: break
            skip += 40
            time.sleep(random.uniform(0.2, 0.8)) # Smanjen sleep zbog bržeg rada
    ...
    if not restaurants or stop_event.is_set():
        if not restaurants: _update_city_progress(city_display, status="⚠️ Nije pronađen nijedan restoran.")
        _write_status_file()
        return []

    # ── Faza 2: Paralelno fetchovanje assortment akcija ─────────────────────────────────
    slugs = list(restaurants.keys())
    total = len(slugs)
    completed = 0
    _update_city_progress(city_display, total=total, found=total, status=f"⚡ Učitavam item popuste (0/{total})...")
    _write_status_file()

    with ThreadPoolExecutor(max_workers=FETCH_WORKERS) as executor:
        futures = {
            executor.submit(_fetch_one, slug, primary_lat, primary_lon, restaurants[slug]["_feed_akcije"], stop_event): slug
            for slug in slugs
        }
...
def _write_status_file():
    with _city_progress_lock: data = dict(_city_progress)
    try: Path("_scan_city_progress.json").write_text(json.dumps(data, ensure_ascii=False), encoding="utf-8")
    except Exception: pass

def scan_all_cities(selected_cities: list[str], status_placeholder, stop_event: threading.Event) -> pd.DataFrame:
    with _city_progress_lock: _city_progress.clear()
    for city in selected_cities: _update_city_progress(city, found=0, total=0, status="⏳ Čeka na red...")
    _write_status_file()
    ...
    for i, city in enumerate(selected_cities):
        ...
        try:
            ...
            _write_status_file()
        if i < len(selected_cities) - 1 and not stop_event.is_set(): time.sleep(0.5)
...
def scan_nopromo_cities(selected_cities: list[str], prev_df: pd.DataFrame, stop_event: threading.Event) -> pd.DataFrame:
    ...
    for city in selected_cities:
        ...
        with ThreadPoolExecutor(max_workers=FETCH_WORKERS) as executor:
            futures = {executor.submit(_fetch_one, slug, primary_lat, primary_lon, [], stop_event): slug for slug in slugs}
            for future in as_completed(futures):
                if stop_event.is_set():
                    executor.shutdown(wait=False, cancel_futures=True)
                    break
    ...
    all_parts = []
    if updated_rows: all_parts.append(pd.DataFrame(updated_rows))
    if not had_promo.empty: all_parts.append(had_promo)
    if not other.empty: all_parts.append(other)

    if all_parts: return pd.concat(all_parts, ignore_index=True)
    return prev_df.copy()
...
def send_alert_email(am_email: str, am_name: str, alerts: list[dict]) -> bool:
    try:
        ...
        for a in alerts:
            ...
            if akcije_filtered != "-": akcije_html = akcije_filtered.replace("\n", "<br>")
            else: akcije_html = "<span style='color:#aaa'>–</span>"

            link = a.get("link", "")
            if link: naziv_cell = f"<a href='{link}' style='color:#222;text-decoration:none;font-weight:600'>{a['naziv']}</a>"
            else: naziv_cell = f"<span style='font-weight:600'>{a['naziv']}</span>"
...
def load_scheduler_config() -> dict:
    if SCHEDULER_FILE.exists():
        try: return json.loads(SCHEDULER_FILE.read_text())
        except Exception: pass
    return {"enabled": False, "hour": 8, "minute": 0, "cities": CITIES}

def save_scheduler_config(cfg: dict): SCHEDULER_FILE.write_text(json.dumps(cfg))
...
def run_scheduled_scan_and_send():
    ...
    try: df = scan_all_cities(cfg["cities"], NullPH(), stop_ev)
    except Exception as e:
    ...
    def should_alert(row): return filter_akcije_for_email(row["akcije"]) != "-"
...
if "df_wolt" not in st.session_state: st.session_state.df_wolt = pd.DataFrame()
if "last_scan" not in st.session_state: st.session_state.last_scan = None
if "scan_stop_event" not in st.session_state: st.session_state.scan_stop_event = threading.Event()
if "scan_running" not in st.session_state: st.session_state.scan_running = False
if "scan_start_time" not in st.session_state: st.session_state.scan_start_time = None
if "scan_mode" not in st.session_state: st.session_state.scan_mode = "full"

# ─────────────────────────── UI ──────────────────────────────────────────────

st.title("🏷️ Promo Monitor – Turbo Mode")
st.caption("Munjevito brzo skeniranje: Očitava opšte promocije sa feeda + detektuje item-level sniženja direktno iz asortimana.")
...
with tab_scan:
    ...
    else: no_promo_count = 0

    col_btn, col_btn2, col_stop, col_info = st.columns([1.2, 1.5, 0.9, 2.4])
    with col_btn:
        run_scan = st.button(
            "▶️ Full Scan", type="primary", use_container_width=True,
            disabled=not selected_cities or st.session_state.scan_running,
        )
    with col_btn2:
        run_nopromo = st.button(
            f"🔍 No Promo Scan ({no_promo_count})", use_container_width=True,
            disabled=not selected_cities or st.session_state.scan_running or not nopromo_available or no_promo_count == 0,
        )
    with col_stop:
        stop_scan = st.button(
            "⏹️ Zaustavi", use_container_width=True, disabled=not st.session_state.scan_running, type="secondary",
        )
    with col_info:
        if st.session_state.last_scan:
            st.info(f"⏱️ Poslednji scan: **{st.session_state.last_scan}** | Ukupno: **{len(st.session_state.df_wolt)}**")
        if not selected_cities: st.warning("Izaberi bar jedan grad.")
    ...
    if st.session_state.scan_running and not scan_done_flag:
        ...
        try: status_msg = Path("_scan_status.txt").read_text()
        except Exception: status_msg = "🔄 Skeniranje..."

        st.markdown(f"### 🔄 Skeniranje u toku — {m2:02d}:{s2:02d}")

        try: city_prog = json.loads(Path("_scan_city_progress.json").read_text(encoding="utf-8"))
        except Exception: city_prog = {}
    ...
    if st.session_state.scan_running and scan_done_flag:
        ...
        try: df_result = pd.read_json("_scan_result.json", orient="records")
        except Exception: df_result = pd.DataFrame()
        ...
        if df_result is not None and not df_result.empty:
            ...
            if scan_mode_done == "nopromo":
                ...
                st.success(f"✅ No Promo Scan završen za **{m:02d}:{s:02d}**! Od preskočenih, **{newly_found}** restorana sada ima akcije.")
            else:
                st.success(f"✅ Turbo Scan završen za **{m:02d}:{s:02d}**! Pronađeno **{len(df_result)}** restorana, **{len(df_result[df_result['akcije'] != '-'])}** sa akcijama.")
            st.rerun()
        else:
            if _stop_ev.is_set(): st.warning("⏹️ Scan je zaustavljen pre završetka.")
            else: st.error("❌ Scan nije vratio podatke.")
    ...
    if not df.empty:
        ...
        sa_wolt_plus = len(df[df["akcije"].str.contains(r"Wolt\+", case=False, na=False)])

        for col, val, lbl in [
            (k1, total,       "Ukupno restorana"),
            (k2, sa_akcijama, "Ima akciju"),
            (k3, sa_wolt_plus,"Wolt+ akcije"),
            (k4, otvoreni,    "Trenutno otvoreno"),
            (k5, novi,        "Novi restorani"),
        ]:
            with col:
                st.markdown(f"""
                <div class='kpi'>
        ...
        with fc5: samo_wolt_plus = st.checkbox("💙 Samo Wolt+ akcije", value=False, key="scan_wolt_plus")
        with fc6: samo_otvoreni = st.checkbox("🟢 Samo otvoreni", value=False, key="scan_otvoreni")
        with fc7: samo_snizenja = st.checkbox("🍔 Samo konkretna sniženja jela", value=False, key="scan_snizenja")
        ...
        akcija_filter = st.multiselect("🎯 Filtriraj po akciji:", options=sve_akcije_tekst, default=[], placeholder="Sve akcije...", key="scan_akcija_filter")
        ...
        if akcija_filter:
            mask = fdf["akcije"].apply(lambda cell: any(a in cell for a in akcija_filter) if cell != "-" else False)
            fdf = fdf[mask]
        if samo_wolt_plus: fdf = fdf[fdf["akcije"].str.contains(r"Wolt\+", case=False, na=False)]
        if samo_snizenja: fdf = fdf[fdf["akcije"].str.contains(r"\[Sniženje\]", na=False)]

        total_fdf = len(fdf)
        sa_ak     = len(fdf[fdf["akcije"] != "-"])
        sa_wplus  = len(fdf[fdf["akcije"].str.contains(r"Wolt\+", case=False, na=False)])
        ...
        for col, val, lbl, color in [
            ...
            (cnt3, sa_wplus,  "Wolt+",       "#8e44ad"),
            (cnt4, sa_novi_f, "Novi",        "#e74c3c"),
            (cnt5, sa_otv,    "Otvoreni",    "#2ecc71"),
        ]:
            with col:
                st.markdown(f"""
...
with tab_amm:
    ...
    with a1: sel_rest = st.selectbox("Restoran (iz poslednjeg scana):", ["-- Odaberi --"] + rest_options, key="amm_sel")
    ...
    with b2: amm_name  = st.text_input("Ime AM-a:", placeholder="Marko M.", key="amm_name")
    with b3: amm_email = st.text_input("Email AM-a:", placeholder="marko@firma.com", key="amm_email")
    with b4:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("💾 Sačuvaj", use_container_width=True, key="amm_save"):
            if not final_rest: st.error("Izaberi ili upiši naziv restorana.")
            elif not amm_name or not amm_email: st.error("Upiši ime i email AM-a.")
            else:
                ...
                else: amm_df = pd.concat([amm_df, pd.DataFrame([new_row])], ignore_index=True)
                save_amm(amm_df)
    ...
    if amm_df.empty: st.info("Baza je prazna. Dodaj prvi restoran gore.")
    else:
        ...
        edited = st.data_editor(
            view.reset_index(drop=True), use_container_width=True, num_rows="dynamic", hide_index=True,
            column_config={
        ...
        if st.button("💾 Sačuvaj izmene u tabeli", key="amm_save_tbl"):
            if am_filt == "Svi": save_amm(edited)
            else:
    ...
    st.markdown("---")
    st.markdown("#### 📤 Export restorana")
    if df_wolt.empty: st.info("Pokreni scan prvo.")
    else:
    ...
    if uploaded:
        try:
            ...
            merged = pd.concat([amm_df, new_df], ignore_index=True).drop_duplicates(subset=["restaurant_norm", "city"], keep="last")
            save_amm(merged)
...
with tab_alert:
    ...
    if df_wolt.empty: st.warning("⚠️ Nema scan podataka. Pokreni scan u prvom tabu.")
    elif amm_df.empty: st.warning("⚠️ AMM baza je prazna. Dodaj restorane u drugom tabu.")
    else:
        ...
        if sa_akcijama.empty: st.info("✅ Nijedan partner iz AMM baze trenutno nema relevantne akcije.")
        else:
            ...
            with af2:
                am_filt_a = st.multiselect(
                    "AM:", sorted(sa_akcijama["am_name"].dropna().unique().tolist()),
            ...
            if st.button("🚀 Pošalji alertove", type="primary"):
                ...
                for (am_name, am_email_addr), grp in am_groups:
                    alerts = [
                        {"naziv": row["naziv"], "grad": row["grad"], "akcije": row["akcije"], "link": row.get("link", "")}
                        for _, row in grp.iterrows()
                    ]
                    ok = send_alert_email(am_email_addr, am_name, alerts)
                    if ok:
                    ...
                    else: st.error(f"❌ Greška pri slanju: {am_name} ({am_email_addr})")
...
with tab_stats:
    ...
    if log_df.empty: st.info("Još nema poslatih alerta. Statistika će se pojaviti posle prvog slanja.")
    else:
        ...
        if flog.empty: st.warning("Nema podataka za izabrani period.")
        else:
            ...
            am_stats = (
                ...
                .agg(Slanja=("timestamp", lambda x: x.dt.date.nunique()), Restorana=("restaurant_display", "nunique"),
                     Ukupno_alerta=("restaurant_display", "count"), Poslednji=("timestamp", "max"))
                .reset_index().rename(columns={"am_name": "AM", "am_email": "Email"})
...
with tab_sched:
    ...
    with sc1: sched_enabled = st.toggle("✅ Uključi automatsko slanje", value=cfg.get("enabled", False), key="sched_on")
    with sc2: sched_hour = st.number_input("Sat (0–23):", min_value=0, max_value=23, value=cfg.get("hour", 8), key="sched_hour")
    with sc3: sched_min = st.number_input("Minut (0–59):", min_value=0, max_value=59, value=cfg.get("minute", 0), key="sched_min")

    sched_cities = st.multiselect("Gradovi:", options=CITIES, default=cfg.get("cities", CITIES), key="sched_cities")
    ...
    if st.button("💾 Sačuvaj podešavanja schedulera", type="primary"):
        ...
        st.success(f"✅ Sačuvano! Automatski sken {'UKLJUČEN' if sched_enabled else 'ISKLJUČEN'}.")
...
with tab_debug:
    ...
    with gh_col2:
        if st.button("🔄 Sync AMM", key="sync_amm"):
            amm_fresh = load_amm_github()
            st.success(f"✅ AMM synced: {len(amm_fresh)} redova")
    with gh_col3:
        if st.button("🔄 Sync Alert log", key="sync_log"):
            log_fresh = load_alert_log_github()
            st.success(f"✅ Log synced: {len(log_fresh)} redova")

    st.markdown("---")
    st.markdown("#### ⚙️ Podešavanja fetcha")
    st.info(f"Trenutni broj paralelnih radnika: **{FETCH_WORKERS}** (Turbo Mode dozvoljava više asinhronih poziva jer je Assortment endpoint brz!).")

    st.markdown("---")
    st.markdown("### 🔬 Sirovi API odgovor za restoran (Assortment)")
    ...
    if st.button("🔍 Dohvati sirovi JSON", key="debug_fetch") and debug_slug:
        ...
        url = f"https://consumer-api.wolt.com/consumer-api/consumer-assortment/v1/venues/slug/{debug_slug}/assortment"
        data, status = wolt_get(url)
        if data:
            with st.expander("Pun JSON (Assortment)", expanded=True): st.json(data)
            st.markdown("**Parsirana sniženja:**")
            ...
            for item in data.get("items", []):
                ...
                nova_cena = item.get("price")
                if stara_cena and nova_cena and stara_cena > nova_cena:
                    st.write(f"• {item.get('name')} ({stara_cena//100} -> {nova_cena//100} RSD)")
                    found = True
            if not found: st.warning("Nema parsiranih sniženja.")
        else: st.warning(f"Assortment endpoint nije vratio podatke. HTTP status: {status}")

    st.markdown("---")
    st.markdown("### 📋 Fetch Debug Log")
    col_log1, col_log2 = st.columns([1, 1])
    with col_log1:
        if st.button("🔄 Osveži log", key="refresh_log"): st.rerun()
    with col_log2:
    ...
    try:
        ...
        if log_content.strip():
            ...
            if auth_fails:
                st.error(f"🔐 **{len(auth_fails)} auth grešaka (401/403)**")
            if errors:
                st.warning(f"⚠️ **{len(errors)} ostalih grešaka**")
                with st.expander(f"Ostale greške ({len(errors)})"):
                    st.text("\n".join(errors[:50]))
            if not auth_fails and not errors:
                st.success("✅ Nema grešaka u logu!")
        else: st.info("Log je prazan.")
    except FileNotFoundError: st.info("Log fajl ne postoji.")
