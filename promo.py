    ...
    try:
        ...
        <html><body style='font-family:Arial,sans-serif;background:#f5f5f5;padding:20px'>
          <div style='max-width:560px;margin:auto;background:#fff;border-radius:12px;
            ...
            <p>Detektovan je novi restoran koji nema dodeljenog Account Managera:</p>
            <table style='width:100%;border-collapse:collapse;margin:16px 0'>
              <tr><td style='padding:10px;background:#f9f9f9;font-weight:700;width:35%'>🏪 Restoran</td><td style='padding:10px'>{naziv}</td></tr>
              <tr><td style='padding:10px;background:#f9f9f9;font-weight:700'>📍 Grad</td><td style='padding:10px'>{grad}</td></tr>
              <tr><td style='padding:10px;background:#f9f9f9;font-weight:700'>🔗 Wolt link</td><td style='padding:10px'><a href='{wolt_link}' style='color:#009de0'>{wolt_link}</a></td></tr>
            </table>
...
with tab_scan:
    ...
    if st.session_state.scan_running and scan_done_flag:
        ...
        if df_result is not None and not df_result.empty:
            ...
            save_scan(df_result)
            # Obavesti sales o novim restoranima
                ...
                for _, row in novi_df.iterrows():
                    ...
                    if not has_am:
                        for email in sales_cfg.get(grad, []):
                            ...
                            notified += 1
                if notified:
                    st.info(f"📬 Poslato **{notified}** obaveštenja sales agentima.")
        ...
        if sa_akcijama.empty:
            ...
            if st.button("🚀 Pošalji alertove", type="primary"):
                ...
                for (am_name, am_email_addr), grp in am_groups:
                    alerts = [{"naziv": row["naziv"], "grad": row["grad"], "akcije": row["akcije"], "link": row.get("link", "")} for _, row in grp.iterrows()]
                    ok = send_alert_email(am_email_addr, am_name, alerts)
                    if ok:
                        success_count += 1
